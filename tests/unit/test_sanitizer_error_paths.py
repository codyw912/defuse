...
class TestDocumentSanitizerInitialization:
    ...
    def test_init_with_none_path(self):
        ...
        with pytest.raises(
            DocumentSanitizeError, match="Dangerzone CLI path not provided"
        ):
...
class TestSanitizeMethodErrorPaths:
    ...
    def test_sanitize_nonexistent_input_file(self):
        ...
        try:
            ...
            with pytest.raises(
                DocumentSanitizeError, match="Input file does not exist"
            ):
        ...
        try:
            ...
            with patch("subprocess.run") as mock_run:
                mock_run.side_effect = FileNotFoundError(
                    "dangerzone-cli: command not found"
                )

                with pytest.raises(
                    DocumentSanitizeError, match="Dangerzone CLI not found"
                ):
                    sanitizer.sanitize(input_file)
        finally:
            dangerzone_path.unlink(missing_ok=True)
        ...
        try:
            ...
            with patch("subprocess.run") as mock_run:
                ...
                with patch.object(sanitizer, "validate_output", return_value=False):
                    ...
                    with pytest.raises(
                        DocumentSanitizeError, match="Output file failed validation"
                    ):
                        sanitizer.sanitize(input_file)
        finally:
            dangerzone_path.unlink(missing_ok=True)
        ...
        try:
            ...
            with patch("subprocess.run") as mock_run:
                ...
                with pytest.raises(
                    ...
                    match="Dangerzone did not create expected output",
                ):
                    sanitizer.sanitize(input_file)
        finally:
            dangerzone_path.unlink(missing_ok=True)
            ...
            with patch("subprocess.run") as mock_run:
                ...
                with patch.object(sanitizer, "validate_output") as mock_validate:
                    ...
                    with patch("pathlib.Path.unlink") as mock_unlink:
                        with pytest.raises(
                            DocumentSanitizeError, match="Sanitization error"
                        ):
                            sanitizer.sanitize(input_file)
